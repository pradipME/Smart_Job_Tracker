...
function SkillBadge({ skill, match }: { skill: string; match?: 'has' | 'missing' }) {
  return (
    <span className={`inline-flex items-center gap-1 px-2 py-1 text-[11px] font-medium border ${
      match === 'has'
...
function ExpandableSection({ title, children }: { title: string; children: React.ReactNode }) {
  ...
  return (
    <div className="border border-slate-200">
      <button onClick={() => setOpen(!open)} className="flex items-center justify-between w-full px-3 py-2 text-xs font-semibold text-slate-700 hover:bg-slate-50 transition-colors cursor-pointer">
        {title}
        {open ? <ChevronUp className="h-3.5 w-3.5 text-slate-400" /> : <ChevronDown className="h-3.5 w-3.5 text-slate-400" />}
      </button>
      {open && <div className="px-3 py-2.5 border-t border-slate-100">{children}</div>}
    </div>
  );
}
...
function TabParseResume() {
  ...
  return (
    <AiCard title="Resume Parsing" icon={<Upload className="h-3.5 w-3.5" />}
      subtitle="Upload a PDF resume to extract structured information">
      <div className="space-y-4">
        <div className="flex items-center gap-3">
          <input type="file" accept=".pdf" onChange={(e) => setFile(e.target.files?.[0] || null)}
            className="flex-1 text-xs file:mr-3 file:py-1.5 file:px-3 file:border file:border-slate-200 file:bg-white file:text-xs file:font-medium file:cursor-pointer hover:file:bg-slate-50" />
        ...
        {result && (
          <div className="space-y-3">
            <div className="flex items-center gap-3 p-3 bg-indigo-50 border border-indigo-100">
            ...
            <ExpandableSection title={`Education (${result.education?.length || 0})`}>
              <div className="space-y-1.5">
                {result.education?.map((e, i) => (
                  <div key={i} className="flex items-center gap-2 p-2 bg-slate-50 border border-slate-100">
            ...
            <ExpandableSection title={`Experience (${result.experience?.length || 0})`}>
              <div className="space-y-1.5">
                {result.experience?.map((e, i) => (
                  <div key={i} className="p-2.5 bg-slate-50 border border-slate-100">
                    <div className="flex items-start gap-2">
                      <Briefcase className="h-3.5 w-3.5 text-violet-500 shrink-0 mt-0.5" />
                      <span className="text-sm text-slate-700">{e}</span>
                    </div>
                  </div>
                ))}
...
function TabMatchResume() {
  ...
  return (
    <AiCard title="Resume Match" icon={<FileText className="h-3.5 w-3.5" />}
      subtitle="Compare resume against job description">
      <div className="space-y-4">
        <textarea value={resumeText} onChange={(e) => setResumeText(e.target.value)} rows={4}
          placeholder="Paste resume text here..." className="flex w-full border border-slate-200 bg-white px-3 py-2 text-sm placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-400 resize-none" />
        <textarea value={jobDesc} onChange={(e) => setJobDesc(e.target.value)} rows={4}
          placeholder="Paste job description here..." className="flex w-full border border-slate-200 bg-white px-3 py-2 text-sm placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-400 resize-none" />
        ...
        {result && (
          <div className="space-y-4">
            <div className="flex flex-col items-center gap-2 p-4 border border-slate-200 bg-slate-50">
            ...
            <ExpandableSection title={`Strengths (${result.strengths?.length || 0})`}>
              <div className="space-y-1.5">
                {result.strengths?.map((s, i) => (
                  <div key={i} className="flex items-start gap-2 p-2 bg-emerald-50 border border-emerald-100">
            ...
            <ExpandableSection title={`Weaknesses (${result.weaknesses?.length || 0})`}>
              <div className="space-y-1.5">
                {result.weaknesses?.map((w, i) => (
                  <div key={i} className="flex items-start gap-2 p-2 bg-red-50 border border-red-100">
...
function TabRecruiterSummary() {
  ...
  return (
    <AiCard title="AI Recruiter Summary" icon={<MessageSquare className="h-3.5 w-3.5" />}
      subtitle="Generate 2-3 paragraph professional summary">
      <div className="space-y-4">
        <textarea value={resumeText} onChange={(e) => setResumeText(e.target.value)} rows={4}
          placeholder="Paste resume text..." className="flex w-full border border-slate-200 bg-white px-3 py-2 text-sm placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-400 resize-none" />
        <textarea value={jobDesc} onChange={(e) => setJobDesc(e.target.value)} rows={3}
          placeholder="Paste job description (optional)..." className="flex w-full border border-slate-200 bg-white px-3 py-2 text-sm placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-400 resize-none" />
        ...
        {result && (
          <div className="p-4 bg-indigo-50 border border-indigo-100">
            <div className="flex items-center gap-2 mb-3">
              ...
              <p className="text-[10px] font-semibold text-slate-500 uppercase tracking-wider">Recruiter Assessment</p>
            </div>
...
function TabRankCandidates() {
  ...
  return (
    <AiCard title="Candidate Ranking" icon={<BarChart3 className="h-3.5 w-3.5" />}
      subtitle="AI-powered candidate ranking for any job">
      <div className="space-y-4">
        <textarea value={jobDesc} onChange={(e) => setJobDesc(e.target.value)} rows={3}
          placeholder="Paste job description..." className="flex w-full border border-slate-200 bg-white px-3 py-2 text-sm placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-400 resize-none" />

        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <p className="text-[10px] font-semibold text-slate-500 uppercase tracking-wider">Candidates ({candidates.length})</p>
            <Button size="sm" variant="ghost" onClick={addCandidate}>+ Add</Button>
          </div>
          {candidates.map((c, idx) => (
            <div key={idx} className="border border-slate-200 p-3 space-y-2">
              <div className="flex items-center justify-between">
              ...
              <input type="text" value={c.name} onChange={(e) => updateCandidate(idx, 'name', e.target.value)}
                placeholder="Candidate name" className="flex h-8 w-full border border-slate-200 bg-white px-3 text-xs focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-400" />
              <textarea value={c.resumeText} onChange={(e) => updateCandidate(idx, 'resumeText', e.target.value)} rows={2}
                placeholder="Paste resume text..." className="flex w-full border border-slate-200 bg-white px-3 py-1.5 text-xs focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-400 resize-none" />
            </div>
          ))}
        </div>
        ...
        {result?.rankedCandidates && result.rankedCandidates.length > 0 && (
          <div className="space-y-2">
            <p className="text-[10px] font-semibold text-slate-500 uppercase tracking-wider">Ranked Results</p>
            {result.rankedCandidates.map((rc, idx) => (
              <div key={rc.id} className="border border-slate-200 p-3 hover:bg-slate-50 transition-colors">
                <div className="flex items-center gap-3">
                  <div className={`flex h-8 w-8 shrink-0 items-center justify-center font-bold text-xs border ${
                    idx === 0 ? 'bg-amber-100 text-amber-700 border-amber-200' :
                    idx < 3 ? 'bg-indigo-50 text-indigo-700 border-indigo-200' : 'bg-slate-100 text-slate-500 border-slate-200'
                  }`}>#{idx + 1}</div>
...
function TabGenerateQuestions() {
  ...
  return (
    <AiCard title="AI Interview Questions" icon={<HelpCircle className="h-3.5 w-3.5" />}
      ...
      <div className="space-y-4">
        ...
        <textarea value={resumeText} onChange={(e) => setResumeText(e.target.value)} rows={3}
          placeholder="Paste resume text (optional)..." className="flex w-full border border-slate-200 bg-white px-3 py-2 text-sm placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-400 resize-none" />
        <Button onClick={handleGenerate} loading={mutation.isPending} className="w-full">
        ...
        {result?.questions && (
          <div className="space-y-3">
            {(['technical', 'behavioral', 'hr'] as const).map(cat => (
              <div key={cat} className="border border-slate-200">
                <div className={`px-3 py-1.5 text-[10px] font-semibold uppercase tracking-wider border-b ${
                  cat === 'technical' ? 'bg-indigo-50 text-indigo-700 border-indigo-100' :
                  cat === 'behavioral' ? 'bg-violet-50 text-violet-700 border-violet-100' :
                  'bg-emerald-50 text-emerald-700 border-emerald-100'
                }`}>{cat} Questions</div>
                <div className="p-3 space-y-1.5">
                  {result.questions[cat]?.map((q, i) => (
                    <div key={i} className="flex items-start gap-2 p-2 hover:bg-slate-50 transition-colors">
...
function TabCoverLetterReview() {
  ...
  return (
    <AiCard title="Cover Letter Review" icon={<FileEdit className="h-3.5 w-3.5" />}
      ...
      <div className="space-y-4">
        <textarea value={coverLetter} onChange={(e) => setCoverLetter(e.target.value)} rows={6}
          placeholder="Paste cover letter here..." className="flex w-full border border-slate-200 bg-white px-3 py-2 text-sm placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-400 resize-none" />
        ...
        {result && (
          <div className="space-y-3">
            <div className="flex items-center gap-2 p-3 bg-amber-50 border border-amber-100">
              ...
              <div>
                <p className="text-[10px] text-slate-500">Professionalism Score</p>
            ...
            <ExpandableSection title={`Suggestions (${result.suggestions?.length || 0})`}>
              <div className="space-y-1.5">
                {result.suggestions?.map((s, i) => (
                  <div key={i} className="flex items-start gap-2 p-2 bg-indigo-50 border border-indigo-100">
...
function TabSkillGap() {
  ...
  return (
    <AiCard title="Skill Gap Analysis" icon={<Target className="h-3.5 w-3.5" />}
            ...
            <ExpandableSection title={`Learning Recommendations (${result.recommendations?.length || 0})`}>
              <div className="space-y-1.5">
                {result.recommendations?.map((r, i) => (
                  <div key={i} className="flex items-start gap-2 p-2.5 bg-indigo-50 border border-indigo-100">
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h1 className="text-lg font-bold text-slate-900 tracking-tight">AI Recruitment Intelligence</h1>
          <p className="text-xs text-slate-500 mt-0.5">Powered by OpenAI &mdash; Analyze, match, and rank candidates</p>
        </div>
      </div>

      <div className="flex overflow-x-auto gap-px bg-slate-200 border border-slate-200">
        {TABS.map(tab => {
          const Icon = tab.icon;
          return (
            <button key={tab.id} onClick={() => setActiveTab(tab.id)}
              className={`flex items-center gap-1.5 px-3 py-2 text-xs font-medium whitespace-nowrap transition-colors cursor-pointer ${
                activeTab === tab.id
