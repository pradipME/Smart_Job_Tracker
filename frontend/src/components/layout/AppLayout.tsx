...
import {
  LayoutDashboard, Briefcase, Building2, LogOut, Menu, X,
  Bell, Search, Users, ClipboardList, CalendarDays, Sparkles, ChevronDown,
  ChevronRight, ChevronLeft, FileText, CheckSquare, Clock, Settings, BarChart3,
  Plus, User,
} from 'lucide-react';
...
function AdminBreadcrumbs() {
  ...
  const segments = useMemo(() => {
    ...
    if (path.startsWith('/admin/applications')) {
      ...
      parts.push({ label: 'Job Listings', href: '/admin/jobs' });
      ...
      {segments.map((seg, i) => (
        <Fragment key={i}>
          ...
          {seg.href ? (
            ...
            <span className="text-slate-800 font-semibold">{seg.label}</span>
          )}
        </Fragment>
      ))}
    </nav>
...
function AdminTopbar() {
  ...
  return (
    <header className="sticky top-0 z-30 flex h-14 items-center justify-between bg-white border-b border-slate-200 px-5 shrink-0">
      ...
      <div className="flex items-center gap-2.5">
        ...
        <div className="flex h-8 w-8 items-center justify-center rounded-md bg-blue-600 text-white text-xs font-semibold cursor-pointer select-none hover:bg-blue-700 transition-colors">
          A
        </div>
      </div>
    </header>
  ...
  if (isAdmin) {
    return (
      <div className="flex h-screen overflow-hidden bg-slate-50">
        ...
        {/* Enterprise Sidebar - 260px */}
        <aside className="fixed inset-y-0 left-0 z-50 flex w-[260px] flex-col bg-[#0B1120] border-r border-slate-800/50 lg:relative">
          {/* Logo */}
          <div className="flex items-center h-14 shrink-0 px-4 border-b border-slate-800/50">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                ...
                <span className="text-[9px] font-medium text-blue-400 block leading-tight tracking-wider uppercase">Enterprise</span>
              </div>
            </div>
            <button
              onClick={() => setSidebarOpen(false)}
              className="flex h-7 w-7 items-center justify-center rounded-md text-slate-400 hover:bg-slate-800 lg:hidden ml-auto"
            >
          ...
          <div className="flex-1 overflow-y-auto px-3 py-4 space-y-5">
            {adminNavGroups.map((group) => (
              <div key={group.label}>
                <p className="px-3 text-[10px] font-semibold uppercase tracking-[0.1em] text-slate-500 mb-1.5">
                  {group.label}
                </p>
          ...
          <div className="border-t border-slate-800/50 px-3 py-2.5 space-y-0.5">
            <div className="flex items-center gap-3 px-3 py-2 rounded-md">
              <div className="flex h-8 w-8 items-center justify-center rounded-md bg-blue-600/20 text-blue-400 text-sm font-semibold">
                {auth.role === 'ADMIN' ? 'A' : <User className="h-3.5 w-3.5" />}
              </div>
            ...
            <button
              ...
              className="flex w-full items-center gap-3 rounded-md px-3 py-2 text-sm font-medium text-slate-400 hover:bg-red-500/10 hover:text-red-400 transition-all duration-150"
            >
              <div className="flex h-7 w-7 items-center justify-center rounded-md bg-slate-800/50 text-slate-400">
        ...
        {!sidebarOpen && (
          <button
            ...
            className="fixed bottom-4 left-4 z-30 flex h-9 w-9 items-center justify-center rounded-lg bg-[#0F172A] text-white shadow-lg lg:hidden"
          >
